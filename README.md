...
Calcula radiación solar horaria para los cuatro días representativos de las estaciones, aporte anual, horas sol equivalentes y comparación por hora. El usuario ingresa mes y día; la aplicación obtiene automáticamente el día del año `n` y calcula la declinación con `δ = 23,45° · sen[(360° / 365) · (n − 81)]`. Incluye además la inclinación óptima de paneles mediante `β = |L − δ|`, con latitud y declinación editables; la calculadora de ángulo horario y altitud solar: `H = 15°(hora solar verdadera − 12)` y `sen(βₙ) = cos(L)cos(δ)cos(H) + sen(L)sen(δ)`; y la calculadora de azimut solar `cos(φₛ) = [sen(βₙ)sen(L) − sen(δ)] / [cos(βₙ)cos(L)]`, seguida de `φₛ = arccos(cos(φₛ))`. En azimut, la fecha seleccionada calcula `δ` y un menú muestra cada hora como `hora terrestre / hora solar verdadera`, ajustada con longitud, huso UTC y ecuación del tiempo; la altitud se obtiene automáticamente para esa hora. Los parámetros de latitud, irradiancia pico, atmósfera, orientación, inclinación y superficie son editables. Incluye una animación de la esfera solar dentro de un cubo celeste. También calcula calor sensible con `Q = m · c · ΔT` y calor latente con `Q = m · L`. Incluye plantillas de calor específico para agua, hielo, vapor de agua, aluminio, vidrio, hierro, cobre y aire, además de relaciones de fusión, solidificación, vaporización, condensación, sublimación y sublimación inversa. La relación sugerida se actualiza al cambiar la plantilla de material.
